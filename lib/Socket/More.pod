...
Intended as a replacement for L<Socket>, implementing only 'modern subset' of
...
Note this is a subset of L<Socket>. Currently the multicast routines are not
implemented. The 'old school' inet_* functions are deliberately not included,
to encourage the usage of getnameinfo/getaddrinfo.
...
Contains all the networking constants (ie C<AF_INET>, C<NI_NUMERICHOST>, etc) for
your platform.

=item L<Socket::More::Lookup>

Implements and exports C<getaddrinfo>, C<getnameinfo> and C<gai_strerror>, with a different
calling convention then perl core Socket implementation.

=item L<Socket::More::Interface>

Implements and exports C<getifaddrs>, C<if_nametoindex>, C<if_indextoname> and C<if_nameindex>
to query the interfaces of your system
...
From version v0.5.0 the structure of the module has been slit into other
modules. The same API is accessable from this module, as it imports them and
